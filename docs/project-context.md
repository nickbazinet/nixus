...
- **Deployed but not enabled.** `GLOBAL#CONFIG` is absent (so every invoke answers `503 hosted_unavailable`). One premium `USER#<sub>/CONFIG` record exists with `monthly_request_limit=200`. See `docs/runbooks/hosted-ai-rollout.md`
