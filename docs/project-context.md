...
- **The model is `anthropic.claude-sonnet-4-6`, direct, in `eu-west-2` — never an inference profile.** The `us.anthropic.claude-sonnet-4-6` profile rejected the AD-8 pre-reservation gate, while the bare model supports both `CountTokens` and `ConverseStream`. Both calls must always use the same model id through the same region-pinned client. IAM is scoped to the single foundation-model ARN the template derives from those two parameters — never a wildcard, never a profile ARN
...
- **No function-level concurrency reservation.** By explicit user decision for the pre-revenue build, the Lambda uses the account's shared 50-concurrency pool. API Gateway throttling (10 RPS / burst 20), per-user/global request caps, and server-owned token ceilings remain the cost controls. The deploy job asserts `get-function-concurrency` has no reservation key, distinguishing this from reserved concurrency `0` (which makes a function inert)
