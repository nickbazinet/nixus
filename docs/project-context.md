...
- **Bound acceptance in BOTH file bytes and pixel dimensions.** `projects/image.rs` enforces 4 MiB *and* 12 MP: the byte ceiling alone does not protect the renderer, because a highly compressible 20000x20000 PNG sits far under it while costing ~4 bytes of decoded RGBA per pixel in the webview, where an `<img> onError` handler cannot catch the result. Enforce the ceilings in Rust *and* as separately-named SQLite `CHECK` constraints
- **Prove the format from the header, never from the extension.** The extension must match the magic bytes, and the container header is parsed by hand — bounds-checked through `get`, widened before multiplying, failing closed when a dimension cannot be determined — because it runs on the main thread inside a synchronous command where a panic takes the window with it. `ai::attachment` trusts extensions; an image boundary must not
- **Move the bytes across IPC only through a dedicated, lazy, per-entity command.** `get_project_image` returns one owner's image and is called only when that row is expanded. Never put bytes in a list response: `get_projects` returning them would make an N-row list cost N images of IPC and N decoded bitmaps
- **Never let the bytes or their base64 reach an audit log or a log line.** Audit rows for set/remove carry metadata only (filename, mime, byte size). No absolute filesystem path may appear in a persisted row, a returned value, an error message, or a log line — on the success path, not only in refusals
- **The frontend gets no filesystem or asset-protocol access.** No `fs:` capability, no asset protocol, no new plugin: the picker returns a path to Rust and Rust reads the file. The benefit is that the webview never holds a filesystem read primitive — it is **not** a CSP improvement, since `tauri.conf.json` already ships `"csp": null` and this pattern did not change it
- **No image path may call `VACUUM`.** Deleting a BLOB returns its pages to SQLite's freelist, which the next image reuses, so repeated upload/remove cycles stay within one `PRAGMA page_size` of the first cycle's footprint without compaction. `VACUUM` rewrites the whole database and would freeze the window for an operation the user thinks is instant. Accepted consequence: the pre-existing delete-all-data wipe already calls `reclaim_space` (`wal_checkpoint(TRUNCATE); VACUUM;`), so it now compacts image data too and is correspondingly slower — that is the one place the cost is acceptable
- **Measure footprint as `.db` + `-wal` + `-shm`.** The connection runs in WAL mode, so a `.db`-only reading of a live app proves nothing; take sizes only after a genuine connection close or a `wal_checkpoint(TRUNCATE)`
