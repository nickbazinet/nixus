...
> **Unchanged:** `CountTokens` still precedes reservation; model and region remain server-owned and invisible to clients; the API Gateway stack, Lambda, and quota table remain in `us-east-1`; every other decision in this document stands except AD-4's function-level reservation, amended below. Where a passage below still reads `us.anthropic.claude-sonnet-4-6` or "inference profile", this amendment governs.
> **Verified:** `CountTokens` and `ConverseStream` both pass on the direct model. `GLOBAL` stays disabled until the remaining rollout gates clear (`docs/runbooks/hosted-ai-rollout.md`). Any fallback from this model, any other region, or any reintroduction of an inference profile is a further amendment.
>
> **Concurrency amendment — 2026-08-26 (user-approved):** for the pre-revenue build, the Lambda uses the account's shared unreserved pool instead of a function-level reservation. API throttling (10 RPS / burst 20), atomic user/global request caps, and token ceilings remain the v1 cost controls. Revisit reserved concurrency only when real traffic warrants it.
...
**External Integrations:** Cognito (auth, unchanged provider, one new scope), Amazon Bedrock (`anthropic.claude-sonnet-4-6` direct in `eu-west-2`, `CountTokens` + `ConverseStream`), DynamoDB, API Gateway.
