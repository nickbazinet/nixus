...
- **Vehicle catalog (D9):** NHTSA vPIC make/model cache in `{app_data_dir}/vehicle_catalog/` (180-day TTL); background refresh on startup; combobox UX with free-text fallback; IPC: `get_vehicle_catalog_status`, `get_vehicle_makes`, `get_vehicle_models`
...
| FR | Epic | Description |
|----|------|-------------|
| FR49 | Epic 16, 20 | Register standalone vehicle (Epic 20 enhances make/model UX) |
...
### Epic 20: Vehicle Make/Model Catalog
Users pick make/model from an NHTSA-backed catalog when online, with free-text fallback when offline.
**FRs covered:** FR49 (data quality enhancement)
**Architecture:** D9 in architecture-car-maintenance.md
...
## Epic 20: Vehicle Make/Model Catalog (NHTSA vPIC)

Users can select vehicle make and model from an up-to-date catalog when online, with graceful free-text fallback when offline.

**Architecture:** [architecture-car-maintenance.md § D9](architecture-car-maintenance.md#d9-vehicle-makemodel-catalog-nhtsa-vpic--disk-cache)
...
**Given** the app starts with network access and no catalog cache (or cache older than 180 days)
...
**Given** a valid makes cache exists on disk
...
**Given** no catalog cache exists (first launch offline or refresh failed with empty cache)
**When** the user opens Add Vehicle or Edit Vehicle
**Then** make and model remain free-text inputs (current behavior preserved)

**Given** a catalog-backed form is displayed
...
**Given** `get_vehicle_catalog_status`, `get_vehicle_makes`, and `get_vehicle_models` Tauri commands
